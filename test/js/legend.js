...
(function (V) {
  ...
  UI.SYMBOLS = () => [
    ...
    { id: 'storm', name: 'Nuvola', tool: 'storm', glyph: glyph('storm', { text: 'problema…', muda: 'attesa' }, { pad: 4, extraB: 12 }),
      body: 'Un problema del processo, mai di una persona: etichettala con muda e regola violata. Mettila sul punto esatto dove il flusso si rompe. Errore tipico: nuvole generiche («comunicazione scarsa») — senza muda e regola non è analisi, è un’opinione.<br>La <b>forma si sceglie</b> (nuvola, cerchio, quadrato, triangolo): è comodo per dire a colpo d’occhio «questi problemi sono la stessa famiglia», ma il significato non cambia — è sempre un problema di processo. Quando il foglio si affolla, il problema si può ridurre al <b>segno con la «i»</b>: un tocco apre il testo, due il pannello. Nasconderlo non è cancellarlo.',
      vars: (V.STORM_SHAPES || []).map(f => `<span>${glyph('storm', { text: 'problema…', shape: f }, { pad: 4 })}${f}</span>`).join('')
        + `<span>${glyph('storm', { text: 'problema…', shape: 'cerchio', collapsed: true }, { pad: 4, w: 34, h: 30 })}solo il segno</span>` },
